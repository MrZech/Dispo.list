...
BRIDGE="${BRIDGE:-vmbr0}"
STORAGE="${STORAGE:-sLot2}"          # change if needed (e.g. local-lvm, local-zfs, etc.)
TEMPLATE_STORAGE="${TEMPLATE_STORAGE:-local}"  # where templates live (usually "local")
DISK_GB="${DISK_GB:-4}"
CORES="${CORES:-1}"
RAM_MB="${RAM_MB:-1024}"
UNPRIVILEGED="${UNPRIVILEGED:-1}"    # 1 = unprivileged, 0 = privileged
START_ON_BOOT="${START_ON_BOOT:-1}"

# Network:
# - Use DHCP by default (simple + reliable)
# - If you want static IP, set:
#   NET_MODE=static IP=192.168.1.80/24 GW=192.168.1.1
...
pvesm status | awk 'NR==1 || $1=="'"$STORAGE"'"' || true
...
apt-get install -y --no-install-recommends \
  ca-certificates curl gnupg git unzip tar \
...
# Composer (install to /usr/local/bin/composer)
...
# Generate DB creds on the host, pass into the CT
DB_NAME="2fauth_db"
DB_USER="2fauth"
...
# Fetch latest 2FAuth release tarball
...
# Configure app URL after we know IP (we’ll set again below if DHCP)
...
# IMPORTANT: Avoid install failure from security advisories gate
composer config audit.block-insecure false

# Prefer deterministic install; if lockfile missing or install fails, fall back to update
...
# Figure out the container IPv4 (works for DHCP + static)
...
# Set APP_URL now that we know IP
